...
AUDIO_EXTENSIONS = {".opus", ".ogg", ".mp3", ".m4a", ".wav", ".aac"}
...
def build_media_index_from_zip(zip_file: zipfile.ZipFile) -> Dict[str, Dict]:
    ...
    for name in zip_file.namelist():
        ...
        elif ext in AUDIO_EXTENSIONS or base_name.lower().startswith("ptt-"):
            entry["kind"] = "audio"
...
def annotate_message_attachment(message: str, media_index: Dict[str, Dict]):
    ...
    if ext in IMAGE_MIME_TYPES:
        fallback_kind = "image"
    elif ext in AUDIO_EXTENSIONS or filename.lower().startswith("ptt-"):
        fallback_kind = "audio"
    else:
        fallback_kind = "file"
...
def build_attachment_html(attachment: Dict | None) -> str:
    ...
    if attachment.get("kind") == "audio":
        return f"""
        <div class="attachment-box">
          <div class="attachment-title">Voice note attached</div>
          <div class="attachment-sub">{filename}{size_html}</div>
          <div class="attachment-sub">Open from media folder</div>
        </div>
        """
    ...
    <div class="attachment-box">
      ...
      <div class="attachment-sub">{filename}{size_html}</div>
      <div class="attachment-sub">File available in media folder</div>
    </div>
    """
...
async def verify_checkout_session(request: Request):
    try:
        ...
        if mode != "subscription":
            raise HTTPException(status_code=400, detail="This session is not a subscription checkout.")

        if payment_status != "paid":
            return {
                "success": False,
                "message": "Payment not marked paid yet.",
        ...
        if subscription_id:
            ...
            current_period_end = to_iso_from_unix(subscription.get("current_period_end"))
